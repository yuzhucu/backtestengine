...
class BollStrategyReverse(StrategyCompareDay):
    def initialize(self):
        ...
        self.context.run_info.strategy_name = 'boll30s-sr_main-open3'
    ...
    def handle_data(self, data):
        ...
        if boll is not None:
            ...
            if data.close < boll.mb and self.context.direction == SELL and self.context.open_flag:
                self._close(data)
            #
            # if data.close < (self.context.open_price - 7 * self.context.instmt_info['tick_size']) and self.context.direction == BUY and self.context.open_flag:
            #     print('多单止损', self.context.open_price, data.close)
            #     self.context.close_count = 3
            #     self._close(data,type='stoploss')
            #
            # if data.close > (self.context.open_price + 7 * self.context.instmt_info['tick_size']) and self.context.direction == SELL and self.context.open_flag:
    ...
    def _close(self, bar, type=''):
        ...
        if self.context.close_count >= 3:
            ...
            if self.context.open_vol > 0:
                ...
                self.order(self.context.current_contract[0], direction, CLOSE_T, self.context.open_vol, limit_price=open_price, type=type, max_dev = self.context.max_dev)
