...
class BollStrategyReverse(StrategyCompareDay):
    def initialize(self):
        self.context.universe = ['p1801']
        self.runinfo.strategy_name = 'boll30s-p1801-stoploss'
        self.runinfo.feed_frequency = '30s'

        self.runinfo.start_date ='2017-05-02'
        self.runinfo.end_date = '2017-07-30'
        self.runinfo.ip = localip
    ...
    def handle_data(self, data):
        ...
        if boll is not None:
            ...
            if data.close < (self.context.open_price - 15 * self.context.instmt_info['tick_size']) and self.context.direction == BUY and self.context.open_flag:
                print('cccccccccccccccccccccccccccc',self.context.open_price, data.close)
            ...
            if data.close > (self.context.open_price + 15 * self.context.instmt_info['tick_size']) and self.context.direction == SELL and self.context.open_flag:
                print('dddddddddddddddddddddddddddd',self.context.open_price, data.close)
                self.context.close_count = 3
                self._close(data)
