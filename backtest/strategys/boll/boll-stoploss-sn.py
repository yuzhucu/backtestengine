...
class BollStrategyReverse(StrategyCompareDay):
    def initialize(self):
        self.context.universe = ['sn1801']
        self.context.run_info.strategy_name = 'boll30s-sn_main-stoploss15'
        self.context.run_info.feed_frequency = '30s'

        self.context.run_info.start_date = '2017-04-21'
    ...
    def _close(self, bar, type=''):
        ...
        if self.context.close_count >= 3:
            ...
            if self.context.open_vol > 0:
                ...
                self.order(self.context.current_contract[0], direction, CLOSE_T, self.context.open_vol, limit_price=open_price, type=type)
