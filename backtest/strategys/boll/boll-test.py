...
class BollStrategyReverse(BacktestStrategyTest):
    def initialize(self):
        self.context.universe = ['SR801']
        self.context.run_info.strategy_name = 'bollReversePro30s-sr_main-test'
    ...
    def handle_data(self, data):
        ...
        if boll is not None:
            ...
            if self.context.hold_flag:
                if data.close >= data.open and self.context.direction == SHORT:
                    self._close_hold(data)
                elif data.close <= data.open and self.context.direction == LONG:
                    self._close_hold(data)
                else:
                    pass

            if self.context.open_flag and self.context.direction == LONG:
                cur_dev = self.context.open_price - data.low
                max_dev = self.context.max_dev
                self.context.max_dev = max(cur_dev, max_dev)

            if self.context.open_flag and self.context.direction == SHORT:
                cur_dev = data.high - self.context.open_price
                max_dev = self.context.max_dev
                self.context.max_dev = max(cur_dev, max_dev)

            # 突破上轨后跌破中轨多单
            if data.close < boll.mb and self.context.direction == LONG and self.context.can_open_flag and not self.context.hold_flag:
                self._open(data)

            # 突破下轨后涨破中轨开空单
            if data.close > boll.mb and self.context.direction == SHORT and self.context.can_open_flag and not self.context.hold_flag:
                self._open(data)

            # 开过多单后 收盘价超过中轨 平仓计数器 + 1 超过3次平仓
            if data.close > boll.mb and self.context.direction == LONG and self.context.open_flag and not self.context.hold_flag:
                self._close(data)

            # 开过空单后 收盘价跌破中轨 平仓计数器 + 1 超过3次平仓
            if data.close < boll.mb and self.context.direction == SHORT and self.context.open_flag and not self.context.hold_flag:
                self._close(data)

            # if data.close < (self.context.open_price - 15 * self.context.instmt_info['tick_size']) and self.context.direction == LONG and self.context.open_flag:
            #     print('多单止损', self.context.open_price, data.close)
            #     self.context.close_count = 3
            #     self._close(data,type='stoploss')
            #
            # if data.close > (self.context.open_price + 15 * self.context.instmt_info['tick_size']) and self.context.direction == SHORT and self.context.open_flag:
    ...
    def _close(self, bar, type=''):
        ...
        if self.context.close_count >= 3:
            if bar.close >= self.context.open_price and bar.close >= bar.open and self.context.direction == LONG:
                self.context.hold_flag = True
                self.context.temp_price = bar.close
            elif bar.close <= self.context.open_price and bar.close <= bar.open and self.context.direction == SHORT:
                self.context.hold_flag = True
                self.context.temp_price = bar.close
            else:
                open_price = bar.close + self.context.slippage
