...
class Portfolio(object):
    ...
    def update_portfolio(self,price):
        for position in self.positions:
            self.positions[position].update_value_tick(price)
...
# class PortfolioBar(object):
#     def __init__(self):
#
#         self.init_cash = 0
#         self.avail_cash = self.init_cash
...
#         self.netvalue = []
#
#     def update_portfolio_bar(self, bar):
#         for position in self.positions:
#             self.positions[position].update_value_bar(bar)
#         self.upnl = self._sum_upnl()
#         self.pnl = self._sum_pnl()
#         self.equity = self.init_cash + self.upnl + self.pnl
#         self.avail_cash = self.equity - self._sum_marginreq()
...
#             raise ValueError
#         else:
#             self.positions[symbol] = PositionBar(symbol, marginratio)
#             # print('开仓')
#             # print(symbol, direction, vol, price, marginratio)
#             if direction == 'long':
#                 self.positions[symbol].BuyT = BasePositionBar(symbol=symbol, direction=direction, init_vol=vol,
#                                                            init_price=price, marginratio=marginratio)
#             else:
#                 self.positions[symbol].SellT = BasePositionBar(symbol=symbol, direction=direction, init_vol=vol,
#                                                             init_price=price, marginratio=marginratio)
#
#             self.avail_cash = self.avail_cash - self.positions[symbol].marginreq - commission
#             # print(self.positions[symbol].marginreq)
...
#                 raise ValueError
#
#     def modify_position(self, symbol, direction, offset, vol, price, marginratio=0, commissionT=0,
#                         commissionY=0, time='9999'):
#         if offset == 'open':
#             if symbol in self.positions:
#                 self._add_position(symbol=symbol, direction=direction, vol=vol, price=price,
#                                    commission=commissionY)
#             else:  # open new position
#                 self._open_position(symbol=symbol, direction=direction, vol=vol, price=price,
#                                     marginratio=marginratio, commission=commissionY)
#         elif offset == 'CloseT':
#             self._close_position_T(symbol=symbol, direction=direction, vol=vol, price=price,
